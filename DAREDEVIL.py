...
__author__ = 's0md3v'
__version__ = '1.0'
__github__ = 'https://github.com/s0md3v/XSStrike/DAREDEVIL'
  ...
  --module            |    add custom module (e.g. "google.py")
	...
	if opts.proxy:
		...
		if proxy.startswith("http://"):
			...
			logger.error("This Proxy Not Supported from DAREDEVIL")
			exit()
		else:
	...
	if opts.ver:
		...
		exit()
	if opts.url != None and opts.rfile != None:
		logger.error("You Can't Start DAREDEVIL With List option and url option ")
		exit()
	if opts.cookie:
	...
	if opts.module:
		...
		try:
			...
			if name == None or description == None or date == None or emails == None or authors == None:
				logger.error('i need more informations abount module please read the developers docs on DAREDEVIL website')
				exit()
			email = ''
			author = ''
			for i in emails:
